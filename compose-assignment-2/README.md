...
- Remember to clean up after your apt install with `rm -rf /var/lib/apt/lists/*` and use `\` and `&&` properly. You can find examples of them in drupal official image. More on this below under Compose file.
...
- Combine that line with this line, as we need to change permissions on files and don't want to use another image layer to do that (it creates size bloat). This dupal container runs as www-data user but the build actually runs as root; so, often we have to do things like `chown` to change file owners to the proper user: `chown -R www-data:www-data bootstrap`. Remember the fist line needs a `\` at the end to signify the next line is included in the command. And, at the start of the next line, you should have `&&` to signify "if first command succeeds then also run this command".
- Then, just to be safe, change the working directory back to its default (from drupal image) at `/var/www/html`

## Compose File
- We're going to build a custom image in this compose file for drupal service. Start with the Compose file from the previous assignment for Drupal, and we'll add to it, as well as change image name.
- Rename image to `custom-drupal`. We want to make a new image based on the official `drupal:8.2`.
...
- For the `postgres:9.6` service, you need the same password from the previous assignment. Also add a volume for `drupal-data:/var/lib/postgresql/data` so the database will persist across Compose restarts.

## Start Containers, Configure Drupal
- Start containers like before and configure Drupal web install like before.
- After the website comes up, click on `Appearance` in the top bar, and notice a new theme called `Bootstrap` is there. That's the one we added with our custom Dockerfile.
...
- If you exit (ctrl-c) and then `docker-compose` down, it will delete containers but not the volumes; on next `docker-compose up` everything will be as it was.
